 ...
 * 12345, 2    => '1.23e+4'
 */
//! Жесть я сидел выводил формулы, ничего не получилось, а потом я случайно нашел toExponential))) не знал что есть такие методы toExponential
 ...
 * 12.345, 4   => '12.35'
 */
//! Не знал до этого что есть встроенные методы такие как toPrecision
function toPrecision(number, precision) {
  const fixedNum = +number.toFixed(precision);
  return fixedNum.toPrecision(precision);
}
 ...
 * Number(-5)    => -5
 */
//! почему не срабатывает return +number, ведь он тоже делает  переменную простым числом????
